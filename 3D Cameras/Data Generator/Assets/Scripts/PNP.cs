...
public class PNP : MonoBehaviour {
    ...
    /*OpenCV PNP function */
    [DllImport("ComputeRT", EntryPoint = "solvePNPUnity")]
    private static extern int solvePNPUnity(Vec3 position, Vec4 rotation, Mat cameraIntrinsecs, Vec3[] coordinates3D, Vec2[] coordinates2D, Vec3[] position2, Vec4[] rotation2);
	...
	void Update () {
        ...
        if (Input.GetKeyDown(KeyCode.Space))
        {
            ...
            Vec3[] camera2Position = new Vec3[1];
            Vec4[] camera2Rotation = new Vec4[1];
            solvePNPUnity(position, rotation, cameraIntrinsics, coordinates3D, coordinates2D, camera2Position, camera2Rotation);
            ...
            Debug.Log(string.Format("Position Local Unity {0} {1} {2} - Position OpenCV {3} {4} {5}",
                                    ...
                                    camera2Position[0].x, camera2Position[0].y, camera2Position[0].z));
            Debug.Log(string.Format("Rotation Local Unity {0} {1} {2} {3} - Rotation OpenCV {4} {5} {6} {7}",
                                    ...
                                    camera2Rotation[0].x, camera2Rotation[0].y, camera2Rotation[0].z, camera2Rotation[0].w));

            /* Assign the results to the OpenCV camera */
            opencvCamera.transform.localPosition = new Vector3(camera2Position[0].x, camera2Position[0].y, camera2Position[0].z);
            opencvCamera.transform.localRotation = new Quaternion(camera2Rotation[0].x, camera2Rotation[0].y, camera2Rotation[0].z, camera2Rotation[0].w);
        }
    }
}